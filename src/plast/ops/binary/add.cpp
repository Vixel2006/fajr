...
tensor::Tensor AddOperation::execute_cpu(const std::vector<const tensor::Tensor*>& inputs) const
{
    ...
    if (lhs_contiguous_and_matches && rhs_contiguous_and_matches)
    {
        ...
        case core::DType::FLOAT32:
            plast_cpu_add_kernel_float(output.data_as<float>(), lhs.data_as<const float>(),
                                       rhs.data_as<const float>(), num_elements);
            break;
        case core::DType::INT32:
            plast_cpu_add_kernel_int32(output.data_as<int32_t>(), lhs.data_as<const int32_t>(),
    ...
    else
    {
        ...
        {
        case core::DType::FLOAT32:
            plast_cpu_add_kernel_strided_float(output.data_as<float>(), lhs.data_as<const float>(),
...
tensor::Tensor AddOperation::execute_cuda(const std::vector<const tensor::Tensor*>& inputs) const
{
    ...
    // within the kernel launch if needed, or managed by a higher-level CUDA memory manager.
    // For now, we'll pass them as host pointers.
    size_t* output_shape = new size_t[output_shape_vec.size()];
    for (size_t i = 0; i < output_shape_vec.size(); ++i)
    {
        output_shape[i] = output_shape_vec[i];
    ...
    if (lhs_contiguous_and_matches && rhs_contiguous_and_matches)
    {
        ...
        {
        case core::DType::FLOAT32:
            plast_cuda_add_kernel_float(output.data_as<float>(), lhs.data_as<const float>(),
                                        rhs.data_as<const float>(), num_elements);
            break;
        case core::DType::INT32:
            plast_cuda_add_kernel_int32(output.data_as<int32_t>(), lhs.data_as<const int32_t>(),
                                        rhs.data_as<const int32_t>(), num_elements);
            break;
        default:
            delete[] output_shape;
