...
void cos_cpu_forward_float_contig_kernel(const float *a, float *c,
                                         u64 num_elements) {
...
void cos_cpu_forward_float_non_contig_kernel(
...
#pragma omp parallel for private(coords)
  for (u64 i = 0; i < num_elements; ++i) {
    ...
    c_data[c_offset] = cosf(a_data[a_offset]);
  }
}

void cos_cpu_backward_float_contig_kernel(const float *dout, const float *a,
...
void cos_cpu_backward_float_non_contig_kernel(
...
#pragma omp parallel for private(coords)
  for (u64 i = 0; i < num_elements; ++i) {
    ...
    if (da_data)
      da_data[da_offset] -= dout_data[dout_offset] * sinf(a_data[a_offset]);
  }
}
...
void cos_cpu_forward(const Tensor **inputs, Tensor *output, ...) {
  ...
  if (is_contiguous(a) && is_contiguous(output)) {
    switch (a->dtype) {
    case FLOAT32:
      cos_cpu_forward_float_contig_kernel((const float *)a->data,
    ...
    switch (a->dtype) {
    case FLOAT32:
      cos_cpu_forward_float_non_contig_kernel(
...
void cos_cpu_backward(Tensor **inputs, const Tensor *output, ...) {
  ...
  if (is_contiguous(a) && is_contiguous(output)) {
    switch (a->dtype) {
    case FLOAT32:
      cos_cpu_backward_float_contig_kernel(
    ...
    switch (a->dtype) {
    case FLOAT32:
      cos_cpu_backward_float_non_contig_kernel(
