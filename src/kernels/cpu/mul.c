...
void mul_cpu_backward_float_contig_kernel(const float *dout, const float *a,
  ...
  for (; i + SIMD_WIDTH - 1 < num_elements; i += SIMD_WIDTH) {
    ...
    if (da) {
      ...
      __m256 new_grad = _mm256_add_ps(a_grad, _mm256_mul_ps(out_grad, b_val));
      _mm256_storeu_ps(da + i, new_grad);
    }
    if (db) {
      __m256 a_val = _mm256_loadu_ps(a + i);
      __m256 b_grad = _mm256_loadu_ps(db + i);
      __m256 new_grad = _mm256_add_ps(b_grad, _mm256_mul_ps(out_grad, a_val));
...
void mul_cpu_forward(const Tensor **inputs, Tensor *output, ...) {
  ...
  if (is_contiguous(a) && is_contiguous(b) && is_contiguous(output)) {
    switch (a->dtype) {
    case FLOAT32:
      mul_cpu_forward_float_contig_kernel((const float *)a->data,
                                          (const float *)b->data,
    ...
    switch (a->dtype) {
    case FLOAT32:
      mul_cpu_forward_float_non_contig_kernel(
...
void mul_cpu_backward(Tensor **inputs, const Tensor *output, ...) {
  const Tensor *a = inputs[0];
  const Tensor *b = inputs[1];
  ...
  if (is_contiguous(a) && is_contiguous(b) && is_contiguous(output)) {
    switch (a->dtype) {
    case FLOAT32:
      mul_cpu_backward_float_contig_kernel(
    ...
    switch (a->dtype) {
    case FLOAT32:
      mul_cpu_backward_float_non_contig_kernel(
