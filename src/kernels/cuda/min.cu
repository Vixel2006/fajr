...
template <u64 block_size>
__global__ void min_cuda_forward_float_contig_kernel(const float *a, float *c,
                                                     u64 num_elements) {
  ...
  float thread_min_val = FLT_MAX;
  while (i < num_elements) {
    float val1 = (i < num_elements) ? a[i] : FLT_MAX;
    float val2 = (i + blockDim.x < num_elements) ? a[i + blockDim.x] : FLT_MAX;
    thread_min_val = fminf(thread_min_val, fminf(val1, val2));
    i += grid_size;
  }
  sdata[tid] = thread_min_val;
  __syncthreads();
...
template <u64 block_size>
__global__ void
min_cuda_forward_float_noncontig_kernel(const float *a_data, const u64 *a_shape,
  ...
  while (i < num_elements) {
    ...
    min_val = fminf(min_val, fminf(val1, val2));
    i += grid_size;
  }
  sdata[tid] = min_val;
  __syncthreads();
...
__global__ void min_cuda_forward_float_dim_contig_kernel(
  ...
  for (u64 i = 0; i < reduce_size; ++i) {
    ...
    min_val = fminf(min_val, a_data[input_offset]);
  }
  u64 output_offset = cuda_get_offset(output_coords, c_strides, c_ndim);
  c_data[output_offset] = min_val;
}

__global__ void min_cuda_forward_float_dim_noncontig_kernel(
    const float *a_data, const u64 *a_shape, const u64 *a_strides, u64 a_ndim,
    float *c_data, const u64 *c_shape, const u64 *c_strides, u64 c_ndim,
    u64 dim, u64 reduce_size, bool keepdim) {
  u64 output_idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void min_cuda_backward_float_dim_contig_kernel(
...
__global__ void min_cuda_backward_float_dim_noncontig_kernel(
    const float *dc_data, const u64 *dc_shape, const u64 *dc_strides,
    u64 dc_ndim, float *da_data, const float *a_data_fwd,
    const float *c_data_fwd, const u64 *da_shape, const u64 *da_strides,
    u64 da_ndim, u64 dim, u64 reduce_size, bool keepdim) {
...
__global__ void min_cuda_backward_float_contig_kernel(const float *dc,
                                                      float *da,
  ...
  float grad = dc[0];
  float min_val = c_data_fwd[0];

  for (u64 i = idx; i < num_elements; i += strides) {
    if (a_data_fwd[i] == min_val) {
      da[i] += grad;
    }
  }
}

__global__ void min_cuda_backward_float_noncontig_kernel(
...
void min_cuda_forward(const Tensor **inputs, Tensor *output, ...) {
  ...
  if (dim == MAX_NDIM + 1) {
    ...
    if (is_contiguous(a)) {
      switch (a->dtype) {
      case FLOAT32:
        min_cuda_forward_float_contig_kernel<block_size>
      ...
      switch (a->dtype) {
      case FLOAT32:
        min_cuda_forward_float_noncontig_kernel<block_size>
    ...
    if (is_contiguous(a)) {
      switch (a->dtype) {
      case FLOAT32:
        min_cuda_forward_float_dim_contig_kernel<<<grid_size, block_size>>>(
            (const float *)a->data, a->shape, a->strides, a->ndim, c_data,
      ...
      switch (a->dtype) {
      case FLOAT32:
        min_cuda_forward_float_dim_noncontig_kernel<<<grid_size, block_size>>>(
            (const float *)a->data, a->shape, a->strides, a->ndim, c_data,
            output->shape, output->strides, output->ndim, dim, reduce_size,
            keepdim);
        break;
...
void min_cuda_backward(Tensor **inputs, const Tensor *output, ...) {
  ...
  if (dim == MAX_NDIM + 1) {
    ...
    if (is_contiguous(a)) {
      switch (a->dtype) {
      case FLOAT32:
        min_cuda_backward_float_contig_kernel<<<grid_size, block_size>>>(
      ...
      switch (a->dtype) {
      case FLOAT32:
        min_cuda_backward_float_noncontig_kernel<<<grid_size, block_size>>>(
    ...
    if (is_contiguous(a)) {
      switch (a->dtype) {
      case FLOAT32:
        min_cuda_backward_float_dim_contig_kernel<<<grid_size, block_size>>>(
      ...
      switch (a->dtype) {
      case FLOAT32:
        min_cuda_backward_float_dim_noncontig_kernel<<<grid_size, block_size>>>(
            dc_data, output->shape, output->strides, output->ndim, da_data,
            a_data_fwd, c_data_fwd, a->shape, a->strides, a->ndim, dim,
            reduce_size, keepdim);
        break;
