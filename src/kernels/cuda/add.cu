...
void add_cuda_backward(Tensor **inputs, const Tensor *output, ...) {
  ...
  if (is_contiguous(a) && is_contiguous(b) && is_contiguous(output)) {
    ...
    case FLOAT32:
      if (a->requires_grad) {
        add_cuda_backward_float_contig_kernel<<<grid_size, block_size>>>(
            (const float *)output->grad->data, (float *)a->grad->data, NULL,
            num_elements);
      }
      if (b->requires_grad) {
        add_cuda_backward_float_contig_kernel<<<grid_size, block_size>>>(
            (const float *)output->grad->data, NULL, (float *)b->grad->data,
            num_elements);
      }
      break;
    default:
