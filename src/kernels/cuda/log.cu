...
__global__ void log_cuda_forward_float_non_contig_kernel(
  ...
  if (linear_idx < num_elements) {
    ...
    c_data[c_offset] = __logf(a_data[a_offset]);
  }
}
...
void log_cuda_forward(const Tensor **inputs, Tensor *output, ...) {
  ...
  if (is_contiguous(a) && is_contiguous(output)) {
    switch (a->dtype) {
    case FLOAT32:
      log_cuda_forward_float_contig_kernel<<<grid_size, block_size>>>(
...
void log_cuda_backward(Tensor **inputs, const Tensor *output, ...) {
  const Tensor *a = inputs[0];
  u64 num_elements = numel(a);
  ...
  if (is_contiguous(a) && is_contiguous(output)) {
    switch (a->dtype) {
    case FLOAT32:
      log_cuda_backward_float_contig_kernel<<<grid_size, block_size>>>(
