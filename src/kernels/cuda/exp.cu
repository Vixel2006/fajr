...
__global__ void exp_cuda_forward_float_contig_kernel(const float *a, float *c,
                                                     u64 num_elements) {
...
__global__ void exp_cuda_backward_float_contig_kernel(const float *dout,
                                                      const float *a, float *da,
                                                      u64 num_elements) {
  u64 idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < num_elements) {
    if (da)
      da[idx] += dout[idx] * __expf(a[idx]);
  }
}

__global__ void exp_cuda_forward_float_non_contig_kernel(
  ...
  if (linear_idx < num_elements) {
    ...
    c_data[c_offset] = __expf(a_data[a_offset]);
  }
}

__global__ void exp_cuda_backward_float_non_contig_kernel(
  ...
  if (linear_idx < num_elements) {
    ...
    if (da_data)
      da_data[da_offset] += dout_data[dout_offset] * __expf(a_data[a_offset]);
  }
}

void exp_cuda_forward(const Tensor **inputs, Tensor *output, ...) {
  ...
  if (is_contiguous(a) && is_contiguous(output)) {
    switch (a->dtype) {
    case FLOAT32:
      exp_cuda_forward_float_contig_kernel<<<grid_size, block_size>>>(
    ...
    switch (a->dtype) {
    case FLOAT32:
      exp_cuda_forward_float_non_contig_kernel<<<grid_size, block_size>>>(
...
void exp_cuda_backward(Tensor **inputs, const Tensor *output, ...) {
  const Tensor *a = inputs[0];
  u64 num_elements = numel(a);
  ...
  if (is_contiguous(a) && is_contiguous(output)) {
    switch (a->dtype) {
    case FLOAT32:
      exp_cuda_backward_float_contig_kernel<<<grid_size, block_size>>>(
    ...
    switch (a->dtype) {
    case FLOAT32:
      exp_cuda_backward_float_non_contig_kernel<<<grid_size, block_size>>>(
